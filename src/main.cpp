...
const unsigned long TS_MIN_INTERVAL = 20000; // >=20s para evitar rate limit
...
// Comandos simples (corrigido uso de aspas)
...
// Atualiza snapshot local (para TS)
...
// Evento on-change (JSON com aspas e concatenação corrigidas)
if (present != lastPresence) {
lastPresence = present;
String status = present ? "ocupada" : "livre";
...
// opcional: força 1 envio ao TS
maybePostTS(true);
}

// heartbeat opcional para TS
maybePostTS(false);
